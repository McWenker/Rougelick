...
class Map(object):
	# stores values for map, does not draw it into game
	#
	# Map.cleared = cleared squares
	...
	def clear_block(self, position):
		# using current player position, sets current square to fully cleared
		# and nearby squares to partially cleared
		x, y = position
		col = y/TILE_SIZE
		row = x/TILE_SIZE
	...
	def clear_treasure(self, position):
		# given position, clears treasure from it and returns that treasure
		x, y = position
		row = x/TILE_SIZE
		col = y/TILE_SIZE
	...
	def print_ascii_map(self):
		# prints ascii map to console, for troubleshooting
	...
	def move_monsters(self):
		...
		for monster in monsters.keys():
			...
			try:
				...
				pass # monsters can run into walls, edges, chest, etc. it consumes their turn
